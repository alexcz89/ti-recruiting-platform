    ...
    <main className="mx-auto max-w-[1600px] px-6 lg:px-10 py-10 space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="space-y-2">
          <div className="h-7 w-44 rounded-md bg-zinc-200 animate-pulse" />
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="rounded-2xl border bg-white/85 p-4 space-y-3 animate-pulse"
          >
          ...
          {Array.from({ length: 6 }).map((_, i) => (
            <li key={i} className="px-4 py-4 grid grid-cols-12 gap-3 animate-pulse">
              ...
              <div className="col-span-2 flex items-center justify-center">
                <div className="h-6 w-10 bg-zinc-200 rounded-full" />
              </div>
              <div className="col-span-2 flex items-center justify-center">
                <div className="h-6 w-10 bg-zinc-200 rounded-full" />
              </div>
