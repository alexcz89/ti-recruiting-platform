...
export default function Step3Benefits({ onNext, onBack }: Props) {
  ...
  function removeOtro(idx: number) {
    setValue("otrosItems", otrosItems.filter((_, i) => i !== idx), { shouldDirty: true });
  }
    ...
    <div className="space-y-6 rounded-xl border border-zinc-200 bg-white p-6 md:p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {BENEFITS.map((b) => {
          ...
          const fullWidth = (isOtros || isVales) && checked;

          return (
            <div key={b.key} className={clsx(fullWidth ? "sm:col-span-2" : "")}>

              {/* Checkbox card */}
              <label className="flex items-center gap-3 rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4 transition cursor-pointer hover:bg-zinc-50 hover:border-zinc-300 dark:hover:bg-zinc-800/40 dark:hover:border-zinc-600 active:scale-[0.98] has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-50 dark:has-[:checked]:bg-emerald-950/20 dark:has-[:checked]:border-emerald-500 has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-emerald-500/40 has-[:focus-visible]:outline-none">
                <input
                  id={`benefit-${b.key}`}
                  type="checkbox"
                  ...
                  checked={checked}
                  onChange={(e) => {
                    setValue("benefits", { ...benefits, [b.key]: e.target.checked }, { shouldDirty: true });
                    if (isOtros && !e.target.checked) setValue("otrosItems", [], { shouldDirty: true });
              ...
              {/* Panel vales: monto mensual */}
              {isVales && checked && (
                <div className="mt-2 rounded-md border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/20 px-4 py-3">
                  <p className="text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-2">
                    Monto mensual de vales de despensa (opcional)
                  </p>
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="w-full sm:w-36 rounded-md border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 px-3 py-1.5 text-sm text-zinc-800 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                    />
              ...
              {/* Panel otros: lista personalizada */}
              {isOtros && checked && (
                <div className="mt-2 rounded-md border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/20 px-4 py-3 space-y-3">
                  <p className="text-xs font-medium text-zinc-600 dark:text-zinc-400">
                    Agrega los beneficios adicionales que ofreces
                  </p>
                      ...
                      {otrosItems.map((item, idx) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            className="text-zinc-400 hover:text-red-500 transition leading-none text-sm"
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <input
                      ...
                      className="flex-1 rounded-md border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 px-3 py-1.5 text-sm text-zinc-800 dark:text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                    />
                    <button
                      type="button"
                      onClick={addOtro}
                      disabled={!otroInput.trim()}
                      className="rounded-md border border-emerald-500 bg-emerald-600 px-4 py-2 sm:py-1.5 w-full sm:w-auto text-sm font-medium text-white hover:bg-emerald-500 disabled:opacity-40 disabled:cursor-not-allowed transition"
                    >
                      + Agregar
                    </button>
                  </div>
                  <p className="text-[11px] text-zinc-400 dark:text-zinc-500">
                    Presiona Enter o el botón para agregar. Puedes agregar varios.
                  </p>
                </div>
              )}
      ...
      <div className="flex justify-between gap-4 pt-6 mt-6 border-t border-zinc-200 dark:border-zinc-800">
        <button
          type="button"
          className="rounded-md border border-zinc-300 dark:border-zinc-700 px-6 py-3 sm:py-2.5 text-sm font-medium min-h-[44px] hover:bg-zinc-50 dark:hover:bg-zinc-800 transition"
          onClick={onBack}
        >
          Atrás
        </button>
