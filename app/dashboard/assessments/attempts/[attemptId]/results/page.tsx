...
async function getResults(attemptId: string): Promise<ResultsData | null> {
  try {
    ...
    const res = await fetch(
      `${getBaseUrl()}/api/assessments/attempts/${encodeURIComponent(
        attemptId
      )}/results`,
      {
        method: "GET",
...
function AnticheatFlags({ anticheat }: { anticheat: Anticheat }) {
  ...
  if (anticheat.tooFast) {
    items.push({
      label: "Velocidad sospechosa",
      value: "Sí — promedio < 5s por pregunta",
      warn: true,
    });
  }
  ...
  if (anticheat.tabSwitches > 0) {
    items.push({
      ...
      value: String(anticheat.tabSwitches),
      warn: anticheat.tabSwitches >= 3,
    });
  }

  if (anticheat.visibilityHidden > 0) {
    items.push({
      label: "Ocultó la pestaña",
      value: String(anticheat.visibilityHidden),
      warn: anticheat.visibilityHidden >= 3,
    });
  }

  if (anticheat.copyAttempts > 0) {
    items.push({
      label: "Intentos de copiar",
      value: String(anticheat.copyAttempts),
      warn: true,
    });
  }

  if (anticheat.pasteAttempts > 0) {
    items.push({
      label: "Intentos de pegar",
      value: String(anticheat.pasteAttempts),
      warn: true,
    });
  }

  if (anticheat.rightClicks > 0) {
    items.push({
      label: "Click derecho",
      value: String(anticheat.rightClicks),
      warn: anticheat.rightClicks >= 5,
    });
  }

  if (anticheat.focusLoss > 0) {
    items.push({
      label: "Pérdidas de foco",
      value: String(anticheat.focusLoss),
      warn: anticheat.focusLoss >= 5,
    });
  }

  if (anticheat.pageHides > 0) {
    items.push({
      label: "Page hides",
      value: String(anticheat.pageHides),
      warn: anticheat.pageHides >= 3,
    });
  }

  if (anticheat.multiSession) {
    items.push({
      label: "Múltiples dispositivos",
      value: "Detectado",
      warn: true,
    });
  }
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-xs text-gray-400 dark:text-gray-500">
          Score: {anticheat.severityScore ?? 0}
        </span>
      </div>
...
function AnswerCard({ answer, index }: { answer: Answer; index: number }) {
  return (
    <div
      className={`rounded-xl border bg-white p-4 dark:bg-gray-900 sm:p-5 ${
        answer.isCorrect
    ...
    >
      <div className="mb-3 flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1.5 flex flex-wrap items-center gap-2">
            ...
            {answer.difficulty && (
              <span
                className={`rounded-full px-2 py-0.5 text-xs font-medium ${difficultyBadge(
                  answer.difficulty
                )}`}
              >
                {answer.difficulty}
      ...
      <div className="ml-0 space-y-2 sm:ml-8">
        {answer.options.map((opt) => {
          ...
          let cls =
            "flex items-start gap-2.5 rounded-lg px-3 py-2 text-sm border transition-colors";
          ...
          if (isCorrectOpt && isSelected) {
            ...
            cls +=
              " bg-emerald-50/50 border-emerald-200 text-emerald-700 dark:bg-emerald-950/20 dark:border-emerald-800 dark:text-emerald-300";
          } else if (!isCorrectOpt && isSelected) {
            cls +=
      ...
      <div className="mx-auto max-w-5xl space-y-6 px-4 py-6 sm:px-6 sm:py-10">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
            <div
              className={`flex h-20 w-20 shrink-0 flex-col items-center justify-center rounded-2xl border-2 sm:h-24 sm:w-24 ${scoreBg(
                score,
                passing
              )}`}
            >
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm">
                ...
                <div className="flex items-center gap-1.5 text-gray-600 dark:text-gray-400">
                  ...
                  <span>
                    ...
                    /{stats.totalQuestions} correctas
                  </span>
                </div>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                    ...
                    <div className="h-1.5 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
                      <div
                        className={`h-full rounded-full transition-all ${
                          Number(pct) >= passing
                            ? "bg-emerald-400"
                            : Number(pct) >= passing * 0.7
                              ? "bg-amber-400"
                              : "bg-red-400"
                        }`}
                        style={{
                          width: `${Math.min(100, Math.round(Number(pct)))}%`,
                        }}
                      />
                    </div>
                  </div>
          ...
          >
            <div className="mb-4 flex items-center gap-2">
              <ShieldAlert
                className={`h-4 w-4 ${
                  hasSuspiciousFlags ? "text-red-500" : "text-gray-400"
                }`}
              />
