...
export default function JobsFilterBar({ locationOptions, sp, dateWindows }: Props) {
  const formRef = React.useRef<HTMLFormElement>(null);

  // Solo auto-submit para selects (no para el texto)
  const onSelectChange: React.FormEventHandler<HTMLFormElement> = (e) => {
    if ((e.target as HTMLElement).tagName === "SELECT") {
      e.currentTarget.requestSubmit();
    }
  };

  // Submit on Enter en el campo de texto
  ...
  // Submit on blur (cuando el usuario hace click afuera)
  const onSearchBlur: React.FocusEventHandler<HTMLInputElement> = () => {
    formRef.current?.requestSubmit();
  };
  ...
  return (
    ...
    >
      ...
      <div className="md:col-span-4">
        <label className={labelClass}>Buscar vacante</label>
